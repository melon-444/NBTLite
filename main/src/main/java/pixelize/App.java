    ...
    public static void main(String[] args) {
        try {
            ...
            int h = image.getHeight(), w = image.getWidth(),
                    pixH = h / pixelContains, pixW = w / pixelContains;
            double hwRate = pixH / pixW;

            if (pixH > pixHMost || pixW > pixWMost)
                if (hwRate > 1) {
                    pixelContains *= ((double) pixH / (double) pixHMost);
                    pixH = pixHMost;
                    pixW = pixHMost / hwRate;

                } else {
                    pixelContains *= ((double) pixW / (double) pixWMost);
                    pixW = pixWMost;
                    pixH = pixWMost * hwRate;
            ...
            int[][][] colorsumRGB = new int[3][pixH][pixW];//0 for r ,1 for g, 2 for b
                //TODO: fix the competition
            Runnable task = () -> {
                
                    for (int i = 0; i < pixH; i++)
                        for (int j = 0; j < pixW; j++) {
                            if(colorsumRGB[type][i][j] != 0) continue;
                            int sum = 0;
                            for(int clr:image.getRGB(i*pixelContains, j*pixelContains, pixelContains, pixelContains, null, 0, pixelContains)){
                                for (int type = 0; type < 3; type++)
                                colorsumRGB[type][i][j] +=(clr>>(8*(2-type)))&0xff;
                            }
                        }
